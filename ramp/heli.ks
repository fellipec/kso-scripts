@lazyglobal off.
// HELI.KS 
// Usage: Type 'RUN HELI.' in kOS console for piloting planes.
...
FUNCTION TakeOff {
    ...
    lights on.
    stage.
    local takeoffthrottle is 0.
    lock throttle to takeoffthrottle.
    until ship:verticalspeed > 20 {
        set takeoffthrottle to takeoffthrottle + 0.1.
        wait 1.
    }    
    local P is PitchLimit().
    LOCK STEERING TO HEADING(MagHeading(), 0).
    wait until ship:altitude > LandedAlt + 150.
    gear off.
    lights off.
    unlock steering.
    wait 0.
